...
lazy_static! {
    ...
    static ref FUNC_CALL_RE: Regex = Regex::new(r"^([a-zA-Z0-9_.]+)\((.*)\)(\.[a-zA-Z0-9_]+)?$").unwrap();
}
...
impl WorkflowExecutor {
    ...
    pub async fn load_mcp_tools(&mut self, config: &JuglansConfig) {
        ...
        for server_conf in &config.mcp_servers {
            match self.mcp_client.fetch_tools(server_conf).await {
                Ok(tools) => { 
                    info!("  ✅ Connected to [{}], found {} tools.", server_conf.name, tools.len()); 
    ...
    async fn process_parameter(&self, param_str: &str, context: &WorkflowContext) -> Result<Value> {
        ...
        if let Some(caps) = FUNC_CALL_RE.captures(clean_param) {
            ...
            let result_val = self.execute_tool_internal(tool_name, &resolved_args, context).await?;
        ...
        if CONTEXT_VAR_RE.is_match(clean_param_no_quotes) {
             let rendered = CONTEXT_VAR_RE.replace_all(clean_param_no_quotes, |caps: &Captures| {
                let raw_path = &caps[1];
                let path = if raw_path.starts_with("ctx.") { &raw_path[4..] } else { raw_path };
                context.resolve_path(path).ok().flatten()
        ...
        match self.rhai_engine.eval_with_scope::<Dynamic>(&mut scope, clean_param) {
            Ok(result) => {
                let json_result = rhai::serde::from_dynamic::<Value>(&result)?;
                Ok(json_result)
            },
            Err(_) => {
                Ok(json!(clean_param_no_quotes))
            }
        }
    }

    pub async fn execute_tool_internal(&self, name: &str, params: &HashMap<String, String>, context: &WorkflowContext) -> Result<Option<Value>> {
        if let Some(tool) = self.builtin_registry.get(name) {
            ...
            let output_str = self.mcp_client.execute_tool(mcp_tool, Value::Object(args)).await?;
    ...
    async fn evaluate_condition_async(&self, script: &str, context: &WorkflowContext) -> Result<bool> {
        let val = self.process_parameter(script, context).await?;
        Ok(val.as_bool().unwrap_or(false))
    }

    async fn run_single_node(self: Arc<Self>, node_idx: NodeIndex, workflow: &Arc<WorkflowGraph>, context: &WorkflowContext) -> Result<Option<Value>> {
        ...
        match &node.node_type {
            ...
            NodeType::Task(action) => {
                ...
                self.execute_tool_internal(&action.name, &rendered_params, context).await
            },
            NodeType::Foreach { item, list, body } => {
                let clean_path = if list.starts_with("ctx.") { &list[4..] } else { list };
                info!("  [Control] Foreach: iterating over '{}' (path: '{}') into '{}'", list, clean_path, item);
                let list_val = context.resolve_path(clean_path)?
                    .ok_or_else(|| anyhow!("Foreach list variable '{}' not found", clean_path))?;
                let array = list_val.as_array()
            ...
            NodeType::Loop { condition, body } => {
                info!("  [Control] Entering while loop with condition: '{}'", condition);
                let mut loop_count = 0;
                loop {
                    if loop_count > 100 { return Err(anyhow!("Loop limit exceeded.")); }
    ...
    pub async fn run(self: Arc<Self>, workflow: Arc<WorkflowGraph>, config: &JuglansConfig) -> Result<()> {
        info!("🚀 Starting Execution: {} (v{})", workflow.name, workflow.version);
    ...
    pub fn execute_graph<'a>(self: Arc<Self>, workflow: Arc<WorkflowGraph>, context: &'a WorkflowContext) -> Pin<Box<dyn Future<Output = Result<()>> + Send + 'a>> {
        Box::pin(async move {
            let total_nodes = workflow.graph.node_count(); 
            if total_nodes == 0 { return Ok(()); }
            let in_degrees: Arc<Mutex<HashMap<NodeIndex, usize>>> = Arc::new(Mutex::new(workflow.graph.node_indices().map(|idx| (idx, workflow.graph.edges_directed(idx, Direction::Incoming).count())).collect()));
            let completed_nodes = Arc::new(Mutex::new(HashSet::new()));
            let ready_queue: Arc<Mutex<VecDeque<NodeIndex>>> = Arc::new(Mutex::new({ let guard = in_degrees.lock().unwrap(); guard.iter().filter(|(_, &degree)| degree == 0).map(|(&idx, _)| idx).collect() }));
            ...
            while completed_nodes.lock().unwrap().len() < total_nodes {
                ...
                if current_batch.is_empty() {
                    ...
                    if completed < total_nodes { info!("Workflow graph execution finished early/deadlocked. ({} / {} nodes ran)", completed, total_nodes); }
                    break;
                }
                info!("--- Starting execution batch of {} parallel nodes ---", current_batch.len());
                for node_idx in current_batch {
                    ...
                    tasks.push(tokio::spawn(async move {
                        ...
                        let node_result = self_clone.clone().run_single_node(node_idx, &workflow_clone, &context_clone).await;
                        let node_succeeded = node_result.is_ok();
                        match node_result {
                            Ok(Some(output)) => {
                                debug!("  [Output] Result for [{}]: {}", node.id, serde_json::to_string(&output).unwrap_or_default());
                                info!("  ✅ Success");
                                context_clone.set(format!("{}.output", node.id), output).unwrap();
                            },
                            Ok(None) => {
                                debug!("  [Output] No primary output for [{}].", node.id);
                                info!("  ✅ Success");
                                context_clone.set(format!("{}.output", node.id), Value::Null).unwrap();
                            }
                            Err(e) => {
                                warn!("  [Output] Error for [{}]: {}", node.id, e);
                                error!("  ❌ Failed: {}", e); 
                                context_clone.set(format!("{}.error", node.id), json!(e.to_string())).unwrap();
                            }
                        }
                        ...
                        for edge in workflow_clone.graph.edges(node_idx) {
                            ...
                            if edge_info.is_error_path {
                                if !node_succeeded { proceed = true; info!("  -> Taking 'on error' path to [{}]", workflow_clone.graph[successor_idx].id); }
                            } else if node_succeeded {
                                if let Some(condition) = &edge_info.condition {
                                    if self_clone.evaluate_condition_async(condition, &context_clone).await.unwrap_or(false) {
                                        proceed = true; info!("  -> Condition TRUE, taking path to [{}]", workflow_clone.graph[successor_idx].id);
                                    } else { info!("  -> Condition FALSE, skipping path to [{}]", workflow_clone.graph[successor_idx].id); }
                                } else { proceed = true; info!("  -> Taking unconditional path to [{}]", workflow_clone.graph[successor_idx].id); }
                            }
                            if proceed {
                                let mut degrees_guard = in_degrees_clone.lock().unwrap();
                                if let Some(degree) = degrees_guard.get_mut(&successor_idx) {
                                    *degree -= 1;
                                    if *degree == 0 { info!("Node [{}] is now ready to run.", workflow_clone.graph[successor_idx].id); ready_queue_clone.lock().unwrap().push_back(successor_idx); }
                                }
                            }
                        }
                    }));
