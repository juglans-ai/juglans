...
| **Chat channels built-in** | Telegram (poll/webhook) / Discord / Feishu (event/webhook) / WeChat — multi-instance, unified `reply()` routing | None | None | None | Many (no-code) | None | None |
...
**Shipped channels, history, MCP, and platform messaging.** Multi-turn Telegram / Discord / Feishu / WeChat are one TOML block away under `[channels.<kind>.<id>]` — `chat_id` auto-derives per platform, history auto-persists, MCP tools attach inline. The same `.jg` runs on every platform unchanged: workflows write `reply()` and the runtime routes back through whichever channel triggered the run via `ChannelOrigin`. **Outbound push is also first-class** for cron / error handlers / cross-platform notifications: `telegram.send_message`, `discord.send_message`, `wechat.send_message`, `feishu.send_message` (plus `typing` / `edit_message` / `react`) work from any node. No other framework in the matrix bundles channels at this layer.
...
- **Unified `juglans serve`** — one process hosts the HTTP API, every configured channel (active + passive ingress), and cron triggers.
...
- You need chat-platform channels (Telegram / Discord / Feishu / WeChat) or SSE streaming out of the box
