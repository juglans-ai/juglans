...
enum StreamLine {
    ...
    #[serde(rename = "result")]
    ResultLine { #[serde(default)] usage: Option<UsagePayload> },
    #[serde(rename = "system")]
    System { #[serde(default)] mcp_servers: Option<Value>, #[serde(default)] tools: Option<Value> },
...
enum ControlRequestBody {
    ...
    CanUseTool { #[allow(dead_code)] tool_name: String },
    #[serde(other)]
    Unknown,
}
#[derive(Debug, Deserialize)]
...
#[derive(Debug, Deserialize)]
struct UsagePayload { input_tokens: Option<u64>, output_tokens: Option<u64> }
...
impl ClaudeCodeProvider {
    ...
    pub fn set_tool_sessions(&mut self, s: Arc<DashMap<String, McpSession>>) { self.tool_sessions = s; }
    fn map_model(model: &str) -> &str {
        let m = model.to_lowercase();
        if m.contains("opus") { "opus" } else if m.contains("haiku") { "haiku" } else { "sonnet" }
    }
    fn build_prompt(history: &[Message]) -> String {
        let mut parts: Vec<(String, String)> = Vec::new();
        for msg in history {
            let content = serde_json::from_value::<Vec<MessagePart>>(msg.parts.clone())
                .map(|ps| ps.iter().filter_map(|p| p.content.clone()).collect::<Vec<_>>().join("\n"))
                .unwrap_or_default();
            if !content.is_empty() { parts.push((msg.role.clone(), content)); }
        }
        if parts.is_empty() { return String::new(); }
        if parts.len() == 1 { return parts[0].1.clone(); }
        parts.iter().map(|(r, c)| {
            let l = match r.as_str() { "user" => "User", "assistant" => "Assistant", "tool" => "Tool Result", _ => "System" };
            format!("[{}]\n{}\n\n", l, c)
        }).collect()
    }
    fn resolve_cwd(&self) -> String {
        if let Some(s) = self.cwd.strip_prefix("~/") {
            if let Some(h) = std::env::var_os("HOME") { return format!("{}/{}", h.to_string_lossy(), s); }
        }
        self.cwd.clone()
    }
}
...
impl LlmProvider for ClaudeCodeProvider {
    async fn stream_chat(
        ...
        let mcp_config_path = if has_tools {
            ...
            tracing::info!("[claude-code] MCP session {} with {} tools", session_id, mcp_tools.len());
            self.tool_sessions.insert(session_id.clone(), McpSession {
                tools: mcp_tools,
                tool_call_tx: tc_tx,
                result_senders,
            });
        ...
        if has_tools {
            cmd.arg("--output-format").arg("stream-json")
                ...
                .arg("--tools=")
                .arg("--mcp-config").arg(mcp_config_path.as_ref().unwrap())
                .arg("--allowed-tools").arg("mcp__jug0__*");
        } else {
        ...
        if let Some(ref sp) = system_prompt {
            if !sp.trim().is_empty() { cmd.arg("--system-prompt").arg(sp); }
        }
        if !has_tools { cmd.arg(&prompt); }
        cmd.current_dir(&real_cwd).env_remove("CLAUDECODE")
            .env_remove("ANTHROPIC_API_KEY")
            .stdout(std::process::Stdio::piped()).stderr(std::process::Stdio::piped());
        if has_tools { cmd.stdin(std::process::Stdio::piped()); }
        else { cmd.stdin(std::process::Stdio::null()); }

        tracing::info!("[claude-code] Spawning (tools={}) prompt_len={}", has_tools, prompt.len());
        ...
        tokio::spawn(async move {
            ...
            while let Ok(Some(line)) = lines.next_line().await {
                ...
                if let Ok(parsed) = serde_json::from_str::<StreamLine>(t) {
                    for ev in translate(parsed) { if tx1.send(ev).is_err() { return; } }
                }
            }
        ...
        tokio::spawn(async move {
            ...
            while let Ok(Some(l)) = lines.next_line().await { if !buf.is_empty() { buf.push('\n'); } buf.push_str(&l); }
            if !buf.is_empty() { let _ = tx2.send(InternalEvent::Error(buf)); }
        });
        let _tx3 = tx;
        ...
        tokio::spawn(async move {
            ...
            if let Some(p) = cleanup_path { let _ = tokio::fs::remove_file(p).await; }
            drop(chunk_tx);
        });
...
fn translate(line: StreamLine) -> Vec<InternalEvent> {
    match line {
        ...
        StreamLine::ControlRequest { request_id, request } => match request {
            ControlRequestBody::CanUseTool { .. } => vec![InternalEvent::PermissionRequest { request_id }],
            _ => vec![],
        },
            ...
            if let Some(content) = message.get("content").and_then(|c| c.as_array()) {
                for b in content {
                    if b.get("type").and_then(|t| t.as_str()) == Some("text") {
                        if let Some(t) = b.get("text").and_then(|t| t.as_str()) { if !t.is_empty() { text.push_str(t); } }
                    }
                }
            }
            if text.is_empty() { vec![] } else { vec![InternalEvent::AssistantText(text)] }
        }
        StreamLine::System { mcp_servers, tools, .. } => {
            tracing::info!("[claude-code] System: mcp={:?} tools={:?}",
                mcp_servers.as_ref().map(|v| v.to_string()), tools.as_ref().map(|v| v.to_string()));
            vec![]
        }
