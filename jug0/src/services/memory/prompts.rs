...
pub fn get_update_memory_messages(
    retrieved_old_memory_dict: &str,
    response_content: &str,
    custom_update_memory_prompt: Option<&str>,
) -> String {
    let update_prompt = custom_update_memory_prompt.unwrap_or(DEFAULT_UPDATE_MEMORY_PROMPT);

    let current_memory_part = if !retrieved_old_memory_dict.is_empty() {
        format!(
            r#"
